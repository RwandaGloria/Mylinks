...
async function getURLData(req, res) {
  ...
  const findUrl = await db.db.URLs.findOne({
    where: {
...
async function getURL(req, res) {
  ...
  const checkURLInDatabase = await db.db.URLs.findOne({
    where: {
      shortURL: `${base}${shortUrl}`
    }
  });
...
async function resetPassword(req, res) {
  ...
  try {
    ...
    if (!findUser) {
      ...
      const tokenInstance = tokensModel.build({
        token: token,
      ...
      //append token to url
      const url = `http://localhost:8099/confirm-acct?${encodeURIComponent(token)}&user=${encodeURIComponent(findUser.id)}`;


      await utils.sendEmail(email, 'Nodemailer Project', `<h1>Hello ${findUser.name}! </h1> <h2> This is your link: ${url}. It expires in two hours. `);
